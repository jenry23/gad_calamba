<template>
	<div class="attachment-list-container d-flex flex-wrap flex-gap pt-3" @click.stop>
		<template v-for="file in attachments">
			<div class="pic-preview" v-if="!file.wasRecentlyCreated" :key="file.id">
				...
				<div class="pic-details">
					<input class="form-control file-name" type="text" :value="file.file_name" disabled />
		...
		<div class="pic-preview" v-for="file in tmpAttachments" :key="file.uuid">
			<div class="pic-image">
				<img :src="file.preview_thumbnail" :alt="file.file_name" :title="file.file_name" :key="file.uuid" />
			</div>
			<div class="pic-details">
				<div class="file-status mt-1">
					<div class="progress" v-show="checkProgress(file) && !isCanceledOrError(file.o)">
						<div
					...
					<div v-show="!checkProgress(file) && !isCanceledOrError(file.o)" class="text-capitalize">
						{{ file.o.status }}
					</div>
				...
				<div class="d-flex justify-content-between">
					<span class="file-size">{{ filesize(file.size) }}</span>
					<button type="button" class="btn btn-round btn-link text-rose" @click="removeTmpFile(file)">
						<i v-show="isUploading(file.o)" class="material-icons" title="Remove"> delete </i>
						<i v-show="!isUploading(file.o)" class="material-icons" title="Cancel"> close </i>
					</button>
				</div>
			</div>
		</div>
